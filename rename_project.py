...
def main():
    ...
    for dirpath, dnames, fnames in os.walk(root_dir):
        ...
        if '.git' in dirpath or '.build' in dirpath or 'build' in dirpath or 'Dist' in dirpath or 'backend/target' in dirpath:
            continue
            
        for f in fnames:
            if f.endswith('.swift') or f.endswith('.sh') or f.endswith('.yml') or f.endswith('.yaml') or f == 'Package.swift' or f == 'Cargo.toml':
